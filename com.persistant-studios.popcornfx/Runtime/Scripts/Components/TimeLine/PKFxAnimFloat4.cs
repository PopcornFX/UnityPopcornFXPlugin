	...
	public class PKFxAnimFloat4 : MonoBehaviour
	{
		...
		void Start()
		{
			if (m_Fx == null)
			{
				...
				if (m_Fx == null)
				{
					Debug.LogError("[PopcornFX] PKFxAnimFloat4 Doesn't have a target nor a PKFxFX component attached to it", this);
					enabled = false;
				}
			}
			if (m_Fx != null)
		...
		void LateUpdate()
		{
			if (m_AttributeId != -1)
			{
				m_Fx.SetAttributeSafe(m_AttributeId, m_Value.x, m_Value.y, m_Value.z, m_Value.w);
			}
		}
