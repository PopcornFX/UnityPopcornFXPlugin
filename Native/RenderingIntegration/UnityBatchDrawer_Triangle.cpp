...
CUnityBatchDrawer_Triangle::CUnityBatchDrawer_Triangle()
...
bool		CUnityBatchDrawer_Triangle::Setup(const CRendererDataBase *renderer, const CParticleRenderMedium *owner, const PopcornFX::CFrameCollector *fc, const CStringId &storageClass)
{
	if (!CRendererBatchJobs_Triangle_CPUBB::Setup(renderer, owner, fc, storageClass))
		return false;
	owner->m_OnRenderMediumActiveStateChanged += FastDelegate<void(CParticleRenderMedium*, bool)>(this, &CUnityBatchDrawer_Triangle::_OnRenderMediumActiveStateChanged);
...
bool	CUnityBatchDrawer_Triangle::AreRenderersCompatible(const PopcornFX::CRendererDataBase *rendererA, const PopcornFX::CRendererDataBase *rendererB) const
{
...
bool CUnityBatchDrawer_Triangle::CanRender(PopcornFX::SRenderContext &ctx) const
{
	(void)ctx;
	return true;
}

void CUnityBatchDrawer_Triangle::BeginFrame(PopcornFX::SRenderContext &ctx)
{
	(void)ctx;
	_ClearBuffers();
}

bool CUnityBatchDrawer_Triangle::EmitDrawCall(PopcornFX::SRenderContext &ctx, const PopcornFX::SDrawCallDesc &toEmit)
{
	(void)ctx; (void)toEmit;
	return true;
}

void CUnityBatchDrawer_Triangle::Destroy()
{
...
void	CUnityBatchDrawer_Triangle::_OnRenderMediumActiveStateChanged(CParticleRenderMedium *renderMedium, bool active)
{
...
void	CUnityBatchDrawer_Triangle::_ClearBuffers()
{
...
void	CUnityBatchDrawer_Triangle::_UpdateThread_ResizeUnityMesh(const SUnityRenderContext &ctx)
{
	(void)ctx;
	PK_SCOPEDPROFILE();
	m_MeshIsValid = true;
...
void	CUnityBatchDrawer_Triangle::_UpdateThread_SetUnityMeshBounds()
{
	PK_SCOPEDPROFILE();
...
bool CUnityBatchDrawer_Triangle::AllocBuffers(SRenderContext &ctx)
{
	const PopcornFX::SRendererBatchDrawPass &drawPass = DrawPass();
	...
	PK_ASSERT((m_BB_Triangle.TotalVertexCount() > 0 && m_BB_Triangle.TotalIndexCount() > 0) || m_BB_Triangle.TotalParticleCount() > 0);
	...
	m_ParticleCount = m_BB_Triangle.TotalParticleCount();

	{
		m_VertexCount = m_BB_Triangle.TotalVertexCount();
		m_IndexCount = m_BB_Triangle.TotalIndexCount();
...
bool CUnityBatchDrawer_Triangle::MapBuffers(SRenderContext &ctx)
{
	const PopcornFX::SRendererBatchDrawPass &drawPass = DrawPass();
	const SUnityRenderContext				&renderContext = static_cast<SUnityRenderContext&>(ctx);
	PK_SCOPEDPROFILE();
	...
	if (!PK_VERIFY(_RenderThread_SetupBuffersTriangles(toMap)))
		return false;
...
bool	CUnityBatchDrawer_Triangle::UnmapBuffers(SRenderContext &ctx)
{
	const SUnityRenderContext				&renderContext = static_cast<SUnityRenderContext&>(ctx);
	PK_SCOPEDPROFILE();
	if (!m_MeshIsValid)
...
bool CUnityBatchDrawer_Triangle::LaunchCustomTasks(SRenderContext &ctx)
{
	(void)ctx;
	if (!m_MeshIsValid)
		return true;
	...
	for (u32 i = 0; i < m_Exec_SAO2AOS.Count(); ++i)
	{
		if (_PrepareCopySOA2AOS(i) && m_Exec_SAO2AOS[i].m_MappedVertexBuffer[i] != null)
			m_BB_Triangle.AddExecAfterPage(&(m_Exec_SAO2AOS[i]));
	}
	return true;
}
...
bool	CUnityBatchDrawer_Triangle::_PrepareCopySOA2AOS(u32 idx)
{
...
bool	CUnityBatchDrawer_Triangle::_RenderThread_AllocPerViewGeomBuffers(const SGeneratedInputs &genInputs)
{
	PK_SCOPEDPROFILE();
	// -----------------------------------------
...
bool	CUnityBatchDrawer_Triangle::_RenderThread_AllocBillboardingBuffers(const SGeneratedInputs &genInputs)
{
	PK_SCOPEDPROFILE();
...
bool	CUnityBatchDrawer_Triangle::_RenderThread_SetupBuffersTriangles(const SGeneratedInputs &toMap)
{
	PK_SCOPEDPROFILE();
	if ((toMap.m_GeneratedInputs & Drawers::GenInput_Indices) != 0)
	{
		m_BBJobs_Triangle.m_Exec_Indices.m_IndexStream.m_Indices = m_ParticleBuffers.m_ViewIndependantGeom.m_Indices;
		m_BBJobs_Triangle.m_Exec_Indices.m_IndexStream.m_Count = m_IndexCount;
		m_BBJobs_Triangle.m_Exec_Indices.m_IndexStream.m_Large = m_UnityMeshInfo.m_LargeIndices == ManagedBool_True;
	}
	if ((toMap.m_GeneratedInputs & Drawers::GenInput_Position) != 0)
	{
		if (!PK_VERIFY(m_ParticleBuffers.m_ViewIndependantGeom.m_Positions.m_Ptr != null))
			return false;
		m_BBJobs_Triangle.m_Exec_PNT.m_Positions = TStridedMemoryView<CFloat3, 0x10>((CFloat3*)(m_ParticleBuffers.m_ViewIndependantGeom.m_Positions.m_Ptr), m_VertexCount, 0x10);
	}
	if ((toMap.m_GeneratedInputs & Drawers::GenInput_Normal) != 0)
	{
		if (!PK_VERIFY(m_ParticleBuffers.m_ViewIndependantGeom.m_Normals.m_Ptr != null))
			return false;
		m_BBJobs_Triangle.m_Exec_PNT.m_Normals = TStridedMemoryView<CFloat3, 0x10>((CFloat3 *)m_ParticleBuffers.m_ViewIndependantGeom.m_Normals.m_Ptr, m_VertexCount, 0x10);
	}
	if ((toMap.m_GeneratedInputs & Drawers::GenInput_Tangent) != 0)
	{
		if (!PK_VERIFY(m_ParticleBuffers.m_ViewIndependantGeom.m_Tangents.m_Ptr != null))
			return false;
		m_BBJobs_Triangle.m_Exec_PNT.m_Tangents = TStridedMemoryView<CFloat4, 0x10>((CFloat4*)m_ParticleBuffers.m_ViewIndependantGeom.m_Tangents.m_Ptr, m_VertexCount, 0x10);
	}
	if ((toMap.m_GeneratedInputs & Drawers::GenInput_UV0) != 0)
	{
		if (!PK_VERIFY(m_ParticleBuffers.m_TexCoords0.m_Ptr != null))
			return false;
		m_BBJobs_Triangle.m_Exec_PNT.m_Texcoords = TStridedMemoryView<CFloat2>(m_ParticleBuffers.m_TexCoords0.m_Ptr, m_VertexCount);
	}

	// View dependent inputs:
	for (u32 i = 0; i < m_BBJobs_Triangle.m_PerView.Count(); ++i)
	{
		const SParticleBuffers::SPerView		&src = m_ParticleBuffers.m_PerViewGeom[i];
		STriangleBatchJobs::SPerView			&dst = m_BBJobs_Triangle.m_PerView[i];
	...
	m_BBJobs_Triangle.m_Exec_CopyField.m_FieldsToCopy = m_ParticleBuffers.m_AdditionalFieldsBuffers;
	return true;
}
