...
void 	SampleParticleTextureTransformed_float(in UnityTexture2D toSample, in float2 uv0, in float2 uv1, in float2 dUVdx, in float2 dUVdy, in float frameLerp, in bool isRGBOnly,
	out float4 outValue)
{
	...
	if (isRGBOnly)
	{
		SampleTextureImplem(toSample, uv0, uv1, frameLerp, color, true);
		outValue.a = color.a;
	}
}
